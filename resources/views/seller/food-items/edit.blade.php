...
@section('title', 'Edit Menu Item - Seller Dashboard')
...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-8">
            ...
            <p class="text-gray-600">Update food item information</p>
        </div>
        ...
        <form action="{{ route('seller.food-items.update', $foodItem) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-lg p-8">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('description', $foodItem->description) }}</textarea>
            </div>

            <div class="mb-6">
                <label class="block text-gray-700 font-medium mb-2">Price (RM) *</label>
                <input type="number" name="price" value="{{ old('price', $foodItem->price) }}" step="0.01" min="0" required
                       class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500">
            </div>
            ...
            <div class="mb-6">
                <label class="block text-gray-700 font-medium mb-2">Current Food Image</label>
                @if($foodItem->image_url)
                    <div class="mb-3">
                        <img src="{{ asset('storage/' . $foodItem->image_url) }}" alt="{{ $foodItem->name }}" class="w-32 h-32 object-cover rounded-lg">
                    </div>
                @endif
                <label class="block text-gray-700 font-medium mb-2">Update Food Image</label>
